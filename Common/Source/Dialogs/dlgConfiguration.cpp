...
static void UpdateDeviceSetupButton(WndForm* pForm,size_t idx /*, const TCHAR *Name*/) {
  ...
  if (wp) {
    ...
    if (dfe) {
      const TCHAR* Name = dfe->GetAsString();
...
static void SetInfoBoxSelector(WndForm* pForm, int item, int mode)
{
  ...
  if (wp) {
    auto dfe = wp->GetDataField();
    if (dfe) {
      FillDataOptionDescription(dfe.get());
      dfe->Sort(0);
...
static void setVariables(WndForm* pForm) {
  ...
  if (wp) {
    ...
    wp->SetReadOnly(ScreenLandscape);
    wp->RefreshDisplay();
  }
  ...
  if (wp) {
    auto dfe = wp->GetDataField();
    if (dfe) {
      dfe->SetAsFloat(iround(UTCOffset/900.0)/4.0);
    }
    wp->RefreshDisplay();
  }
  SetLocalTime(pForm);
  ...
  if (wp) {
    auto dfe = wp->GetDataField();
    if (dfe) {
      dfe->SetAsFloat(iround(Units::ToAltitude(ClipAltitude / 10.0)));
      dfe->SetUnits(Units::GetAltitudeName());
    }
    wp->RefreshDisplay();
  }
  ...
  if (wp) {
    auto dfe = wp->GetDataField();
    if (dfe) {
      dfe->SetAsFloat(iround(Units::ToAltitude(AltWarningMargin / 10.0)));
      dfe->SetUnits(Units::GetAltitudeName());
    }
    wp->RefreshDisplay();
  }
  ...
  if (wp) {
    auto dfe = wp->GetDataField();
    if (dfe) {
      dfe->SetAsInteger(debounceTimeout);
    }
    wp->RefreshDisplay();
  }
...
// #define  ASK_WAYPOINTS
...
#ifdef ASK_WAYPOINTS
      dfe->Set(WaypointsOutOfRange);
#else
      dfe->Set(WaypointsOutOfRange - 1);
#endif
    }
    wp->RefreshDisplay();
  }
  ...
  if (wp) {
    auto dfe = wp->GetDataField();
    if (dfe) {
      dfe->addEnumText(MsgToken<667>()); // LKTOKEN  _@M667_ = "Statute"
      dfe->addEnumText(MsgToken<455>()); // LKTOKEN  _@M455_ = "Nautical"     
      dfe->addEnumText(MsgToken<436>()); // LKTOKEN  _@M436_ = "Metric"
...
static bool OnUser(WndForm * pWndForm, unsigned id) {
  switch(id) {
    case UPDATE_COM_PORT: {
      ...
      if (pWnd) {
        auto dataField = pWnd->GetDataField();
        if(dataField) {
...
// Don't show external sound config if not compiled for this device or not used
  ...
  if(wp) {
#ifdef _WGS84
    auto pDataField = wp->GetDataField();
    if(pDataField) {
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpAutoZoom"));
  if (wp) {
    if (AutoZoom_Config != wp->GetDataField()->GetAsBoolean()) {
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpUTCOffset"));
  if (wp) {
    int ival = iround(wp->GetDataField()->GetAsFloat() * 3600.0);
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpClipAltitude"));
  if (wp) {
    int ival = iround(Units::FromAltitude(wp->GetDataField()->GetAsInteger()) * 10);
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpWindCalcTime")); // 100113
  if (wp) {
    int ival = wp->GetDataField()->GetAsInteger();
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpTaskFinishRadius"));
  if (wp) {
    int ival = iround(Units::FromDistance(wp->GetDataField()->GetAsFloat()));
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpAltArrivMode"));
  if (wp) {
    AltArrivMode = (AltArrivMode_t)(wp->GetDataField()->GetAsInteger());
  }
  #endif

  wp = pForm->FindByName<WndProperty>(TEXT("prpCheckSum"));
  if (wp) {
    CheckSum = (CheckSum_t)(wp->GetDataField()->GetAsInteger());
  }
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpLKVarioVal"));
  if (wp) {
    LKVarioVal = (LKVarioVal_t)(wp->GetDataField()->GetAsInteger());
  }
  wp = pForm->FindByName<WndProperty>(TEXT("prpHideUnits"));
  if (wp) {
  ...
  if (wp) {
    DeclutterMode = (DeclutterMode_t)(wp->GetDataField()->GetAsInteger());
  }
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpTpFilter"));
  if (wp) {
    if (TpFilter != (TpFilter_t)(wp->GetDataField()->GetAsInteger())) {
      TpFilter = (TpFilter_t)(wp->GetDataField()->GetAsInteger());
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpOverColor"));
  if (wp) {
    if (OverColor != (OverColor_t)(wp->GetDataField()->GetAsInteger())) {
      OverColor = (OverColor_t)(wp->GetDataField()->GetAsInteger());
      SetOverColorRef();
    }
  }
  ...
  if (wp) {
    MapBox = (MapBox_t)(wp->GetDataField()->GetAsInteger());
  }
  ...
  if (wp) {
    GlideBarMode = (GlideBarMode_t)(wp->GetDataField()->GetAsInteger());
  }

  wp = pForm->FindByName<WndProperty>(TEXT("prpNewMapDeclutter")); // VENTA6
  if (wp) {
    NewMapDeclutter = (wp->GetDataField()->GetAsInteger());
  }

  wp = pForm->FindByName<WndProperty>(TEXT("prpGPSAltitudeOffset"));
  if (wp) {
    int ival = iround(Units::FromAltitude(wp->GetDataField()->GetAsInteger()) * 1000.0);
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpAverEffTime"));
  if (wp) {
    if (AverEffTime != (wp->GetDataField()->GetAsInteger())) {
      AverEffTime = (wp->GetDataField()->GetAsInteger());
  ...
  if (wp) {
    ArrivalValue = (ArrivalValue_t)(wp->GetDataField()->GetAsInteger());
  }
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpGliderScreenPosition"));
  if (wp) {
    if (MapWindow::GliderScreenPosition != wp->GetDataField()->GetAsInteger()) {
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpTerrainContrast"));
  if (wp) {
    if (iround((TerrainContrast * 100) / 255) != wp->GetDataField()->GetAsInteger()) {
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpTerrainBrightness"));
  if (wp) {
    if (iround((TerrainBrightness * 100) / 255) != wp->GetDataField()->GetAsInteger()) {
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpAlarmGearWarning"));
  if (wp) {
    int ival = iround(wp->GetDataField()->GetAsInteger());
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpAlarmGearAltitude"));
  if (wp) {
    GearWarningAltitude = iround(Units::FromAltitude(wp->GetDataField()->GetAsInteger()) * 1000.0);
  }
...
void UpdateAircraftConfig(WndForm* pForm) {
  auto wp = pForm->FindByName<WndProperty>(TEXT("prpAircraftCategory"));
  if (wp) {
  ...
  wp = pForm->FindByName<WndProperty>(TEXT("prpMaxManoeuveringSpeed"));
  if (wp) {
    int ival = iround(Units::FromHorizontalSpeed(wp->GetDataField()->GetAsFloat())*1000.0);
